...
export class GPath {
    ...
    private onCRSplineCurve(ptStart: number, ptCount: number, t: number, result: Vector2): Vector2 {
        ...
        var adjustedT: number = (t == 1) ? 1 : repeat(t * (ptCount - 4), 1); // Then we adjust t to be that value on that new piece of segment... for t == 1f don't use repeat (that would return 0f);
